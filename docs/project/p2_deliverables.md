...
- [ ] 数据模型：手帐、页面、块（text/dice/poem_slip/divination）已在本地 DB（drift）建模并可迁移；支持按手帐/页查询。
- [ ] 手帐 CRUD：可创建/重命名/删除手帐，可增删页，页内块顺序可管理。
- [ ] 块编辑：可添加文本块；手帐编辑态工具栏可调用工具（含历史选择与现场使用）并插入结果块；块可排序。
- [ ] 基础展示：手帐阅读视图按页展示块列表，各块类型有对应展示方式。
- [ ] 路由与导航：`/journal`、`/journal/:id`、`/journal/:id/page/:pageId` 可深链打开；无效 id 重定向到 `/journal`。
